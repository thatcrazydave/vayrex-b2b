...
cron.schedule('0 * * * *', async () => {
  try {
    ...
    let cursor = '0';
    do {
      const [nextCursor, keys] = await redisConnection.scan(cursor, 'MATCH', 'user:*:active_jobs', 'COUNT', 100);
      cursor = nextCursor;
    ...
    if (allTrackedKeys.length > 0) {
      ...
      const storedValues = await Promise.all(allTrackedKeys.map(k => redisConnection.get(k)));
      ...
      for (let idx = 0; idx < allTrackedKeys.length; idx++) {
        ...
        if (storedCount !== actualCount) {
          Logger.warn('Correcting stale active_jobs counter', { userId, storedCount, actualCount });
          corrections.push(
            actualCount === 0
...
// ── Startup: sync stale free-tier limits ──────────────────────────────────
// Runs once when the server starts. If the free-tier config changed (e.g.
// filesPerUpload was raised from 1 → 3), this patches existing DB records so
// users don't need to log out / re-register to see the updated allowance.
setImmediate(async () => {
  try {
    const User = require('./models/User');
    const freeLimits = await User.getLiveTierLimits('free');
    const result = await User.updateMany(
      {
        subscriptionTier: 'free',
        'limits.filesPerUpload': { $lt: freeLimits.filesPerUpload }
      },
      { $set: { 'limits.filesPerUpload': freeLimits.filesPerUpload } }
    );
    if (result.modifiedCount > 0) {
      Logger.info('Free-tier filesPerUpload synced to current config', {
        modifiedCount: result.modifiedCount,
        newValue: freeLimits.filesPerUpload
      });
    }
  } catch (err) {
    Logger.error('Free-tier limits sync failed', { error: err.message });
  }
});
