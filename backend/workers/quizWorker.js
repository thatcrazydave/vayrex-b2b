...
const worker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      if (s3FileKey) {
        Logger.info(`Fetching file from storage: ${s3FileKey}`);
        fileBuffer = await storageService.download(s3FileKey);
        Logger.info(`Storage fetch complete`, {
          s3FileKey,
        ...
        type = ".txt";
        Logger.info(`Job ${job.id}: Using text input path`, {
      ...
      const isReplicaSet =
        client.topology?.description?.type?.includes("ReplicaSet") ||
      ...
      try {
        if (isReplicaSet) {
          try {
            ...
            Logger.warn("Failed to start transaction session, falling back", {
              error: sErr.message,
            });
            session = null;
          }
        ...
        if (
          dbError.message.includes("Transaction numbers are only allowed on a replica set")
        ) {
      ...
      if (error.message === "DELAYED_BY_CIRCUIT_BREAKER") {
        ...
        if (job?.data?.userId) {
          await decrementUserActiveJobs(job.data.userId).catch((err) => {
            Logger.warn("Failed to decrement active jobs before circuit breaker delay", {
              error: err.message,
            });
          });
        }
      ...
      Logger.error(`Worker error in job ${job.id}:`, {
        error: error.message,
        stack: error.stack,
      });
      // Permanent failures (e.g. upload never landed) should not be retried
...
async function _processMultiFileJob(job) {
  ...
  // ── Step 1: Fetch ZIP bundle from storage (with retry — upload may still be in
  ...
  Logger.info(`[${jobTag}] Fetching ZIP bundle from storage`, { s3BundleKey });
  const _bundleRedisKey = `bundle_upload:${s3BundleKey}`;
  const FETCH_MAX_ATTEMPTS = 8;
  const FETCH_RETRY_DELAYS_MS = [5000, 10000, 15000, 20000, 30000, 45000, 60000];
      ...
      if (_redis) {
        ...
        if (uploadStatus === "failed") {
          throw new UnrecoverableError(
            "Bundle upload failed on the server side — cannot proceed",
          );
        }
      }
    } catch (redisErr) {
      if (redisErr instanceof UnrecoverableError) throw redisErr;
      // Redis unavailable — ignore and fall through to storage probe
    }

    try {
      bundleBuffer = await storageService.download(s3BundleKey);
      Logger.info(`[${jobTag}] Storage bundle fetched (attempt ${attempt})`, {
      ...
      if (notReady && attempt < FETCH_MAX_ATTEMPTS) {
        const delay = FETCH_RETRY_DELAYS_MS[attempt - 1] || 60000;
        Logger.warn(
          `[${jobTag}] Bundle not in storage yet, retrying in ${delay / 1000}s (attempt ${attempt}/${FETCH_MAX_ATTEMPTS})`,
        );
  ...
  if (!bundleBuffer)
    throw new UnrecoverableError(
      "Failed to fetch bundle from storage after all retry attempts",
    );
  ...
  for (const file of filesWithMeta) {
    try {
      ...
      if (file.extension === ".txt") {
        ...
        extractedText = await batchProcessingService.extractFullContent(
          file.data,
          file.extension,
        );
      ...
      // Clean text
      const { cleanedText } = localOcrService.cleanText(
        extractedText,
        `${jobTag}_${file.fileName}`,
      );
  ...
  const batchResult = await batchGenerationService.processBatchedFile({
    ...
    coherenceContext:
      coherenceResult.coherenceLevel !== "all_coherent"
        ? {
  ...
  const _normalizeQType = (raw) => {
    ...
    const t = String(raw)
      .toLowerCase()
      .replace(/[_\s]+/g, "-");
    if (t.includes("fill") || t.includes("blank") || t.includes("gap") || t.includes("cloze"))
      return "fill-in-blank";
    if (
      t.includes("theory") ||
      t.includes("short") ||
  ...
  try {
    if (isReplicaSet) {
      try {
        ...
        Logger.warn("Failed to start transaction for multi-file, falling back", {
          error: sErr.message,
        });
        session = null;
      }
...
async function _performMultiFilePersistence({
  ...
  if (existingQuestions.length > 0) {
    ...
    Logger.info(
      `[DB SUCCESS] Saved ${savedQuestions.length} questions for multi-file ${jobTag}`,
    );
  ...
  // 2. Storage backup of questions
  const s3BackupKey = await _saveBackupToStorage(userId, topic, cleanedQuestions);
...
// --- Helper: Storage question backup (non-critical, mirrors saveBackupToStorage in server.js) ---
async function _saveBackupToStorage(userId, topic, questions) {
  try {
    const backupKey = `question_backups/${userId}/${topic}_${Date.now()}.json`;
    await storageService.upload(backupKey, Buffer.from(JSON.stringify(questions, null, 2)), {
      contentType: "application/json",
    });
    Logger.info(`[STORAGE SUCCESS] Question backup saved`, { backupKey });
    return backupKey;
  } catch (err) {
    Logger.warn("Question backup to storage failed (non-critical)", { error: err.message });
    return null;
  }
}
...
async function _performIdempotentPersistence({
  ...
  if (existingQuestions.length > 0) {
    ...
    savedQuestions = await Question.insertMany(cleanedQuestions, { session });
    Logger.info(
      `[DB SUCCESS] Saved ${savedQuestions.length} new questions to Database for job ${job.id}`,
    );
