...
router.post("/signup",
  ...
  async (req, res) => {
    try {
      ...
      emailService.sendVerificationEmail(user.email, user.username, verificationToken, verificationCode)
...
// Rate limiting: 3 requests per hour (configured in rateLimitConfig.js)
// Applied automatically via global rateLimiter middleware
router.post('/resend-verification', authenticateToken, async (req, res) => {
  try {
    ...
    await emailService.sendVerificationEmail(user.email, user.username, verificationToken, verificationCode);
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    await emailService.sendPasswordResetEmail(user.email, user.username, resetToken, resetCode);
