...
const BOT_UA_PATTERNS = [
  /sqlmap/i, /nikto/i, /nmap/i, /burp\s?suite/i, /owasp/i, /masscan/i,
  /dirbuster/i, /gobuster/i, /wfuzz/i, /ffuf/i, /hydra/i, /metasploit/i,
  /nessus/i, /openvas/i, /acunetix/i, /w3af/i, /zap\/?/i, /nuclei/i,
  /gospider/i, /subfinder/i, /amass/i, /whatweb/i, /wafw00f/i, /scrapy/i,
  /python-requests\/\d/i, /python-urllib/i, /curl\/\d/i, /wget/i,
  /libwww-perl/i, /java\/\d/i, /httpclient/i, /go-http-client/i,
  /axios\/\d/i, /node-fetch/i, /aiohttp/i, /httpx/i, /zgrab/i,
  /censys/i, /shodan/i, /internet\s?measurement/i,
];
...
const HONEYPOT_PATHS = [
  ...
  '/wp-admin', '/wp-login', '/wp-login.php', '/wp-config.php',
  '/phpmyadmin', '/admin/config', '/api/v0/', '/api/internal/',
  '/api/admin/debug', '/api/admin/shell', '/api/debug',
  '/config.json', '/server.js', '/package.json', '/.DS_Store',
  '/etc/passwd', '/api/graphql', '/api/swagger', '/api/docs',
  '/actuator', '/actuator/health', '/debug/vars', '/trace', '/api/__debug',
...
const UPLOAD_ROUTE_PREFIXES = [
  '/api/upload', '/api/ai/generate-from-notes', '/api/ai/summarize',
  '/api/ai/course-outline', '/api/ai/parse-image', '/api/export/pdf',
...
const AUTH_ROUTES = [
  '/api/auth/login', '/api/auth/signup', '/api/auth/firebase-login',
...
function cleanupStores() {
  ...
  const stores = [_ipBurstStore, _authFailStore, _credStuffStore, _404Store,
                  _connCountStore, _endpointCycleStore, _ipBlockStore, _crawlerStore,
                  _signupStore, _bodyHashStore, _blockEscalation];
...
function getClientIP(req) {
  const raw = (req.ip && req.ip !== '::1' && req.ip !== '127.0.0.1' && req.ip !== '::ffff:127.0.0.1')
    ? req.ip
    : getSocketIP(req);
...
function normalizePath(p) {
  return (p || '/').split('?')[0].toLowerCase().replace(/\/+$/, '').replace(/\/+/g, '/') || '/';
}
...
async function blockIP(ip, durationSeconds, layerCode, reason) {
  ...
  const data = { layerCode, reason, blockedAt: Date.now(), expiresAt, escalationLevel: multiplier };

  Logger.error(`[ATTACK] ${layerCode} - IP blocked`, {
    ip, reason, durationMinutes: Math.ceil(actualDuration / 60),
    layerCode, escalation: `${multiplier}x`
  });
...
async function checkL1_BurstFlood(ip) {
  ...
  let count = await redisZAdd(key, crypto.randomBytes(4).toString('hex'), CONFIG.L1_BURST_WINDOW_S);
  if (count === null) count = memoryIncr(_ipBurstStore, key, CONFIG.L1_BURST_WINDOW_S * 1000);
  if (count > CONFIG.L1_BURST_MAX) {
    await blockIP(ip, CONFIG.L1_BLOCK_DURATION_S, 'ATTACK_L1_IP_BURST',
  ...
  if (isRedisReady()) {
    try {
      ...
      if (count && parseInt(count) >= CONFIG.L2_AUTH_MAX_FAILS) {
        await blockIP(ip, CONFIG.L2_AUTH_BLOCK_S, 'ATTACK_L2_AUTH_BRUTE',
...
async function recordAuthFailure(ip, accountIdentifier) {
  ...
  if (isRedisReady()) {
    try { await redisIncr(l2Key, CONFIG.L2_AUTH_WINDOW_S); } catch (_) {}
  }
  ...
  if (uniqueAccounts === null || uniqueAccounts === undefined) {
    uniqueAccounts = memorySetAdd(_credStuffStore, l3Key, accountIdentifier, CONFIG.L3_WINDOW_S * 1000);
  }
  if (uniqueAccounts >= CONFIG.L3_MAX_ACCOUNTS) {
    await blockIP(ip, CONFIG.L3_BLOCK_S, 'ATTACK_L3_CRED_STUFF',
...
async function recordAndCheckL6_PathScan(ip) {
  ...
  if (count >= CONFIG.L6_MAX_404S) {
    await blockIP(ip, CONFIG.L6_BLOCK_S, 'ATTACK_L6_PATH_SCAN',
      `${count} 404 responses`);
    return true;
  }
  return false;
  ...
  if (xff && trustedProxies.length === 0) {
    ...
    if (hops.length > CONFIG.L9_MAX_FORWARDED_HOPS) {
      Logger.warn('[L9] Suspicious XFF chain (no trusted proxies)', {
        xff, hops: hops.length, socketIP: getSocketIP(req)
      });
  ...
  if (xRealIP && trustedProxies.length === 0) {
    ...
    if (xRealIP !== socketIP) {
      Logger.warn('[L9] Spoofed X-Real-IP/Client-IP', {
        claimed: xRealIP, actual: socketIP
      });
      delete req.headers['x-forwarded-for'];
      delete req.headers['x-real-ip'];
...
async function checkL10_CrawlerFingerprint(req, ip) {
  ...
  const path = normalizePath(req.path);
  const isProtected = path.startsWith('/api/user/') || path.startsWith('/api/ai/') ||
  ...
  if (score >= CONFIG.L10_SCORE_THRESHOLD) {
    ...
    if (velocity >= CONFIG.L10_VELOCITY_THRESHOLD) {
      await blockIP(ip, CONFIG.L10_BLOCK_S, 'ATTACK_L10_CRAWLER',
...
async function checkL11_EndpointCycling(req, ip) {
  ...
  if (distinctCount >= CONFIG.L11_MAX_DISTINCT_ENDPOINTS) {
    await blockIP(ip, CONFIG.L11_BLOCK_S, 'ATTACK_L11_API_RECON',
...
async function checkSignupFlood(req, ip) {
  ...
  if (ipCount === null) ipCount = memoryIncr(_signupStore, ipKey, CONFIG.SIGNUP_WINDOW_S * 1000);

  if (ipCount > CONFIG.SIGNUP_MAX_PER_IP) {
    await blockIP(ip, CONFIG.SIGNUP_BLOCK_S, 'ATTACK_SIGNUP_FLOOD',
...
function checkQueryAbuse(req, ip) {
  ...
  for (const [key, value] of Object.entries(query)) {
    if (typeof value === 'string') {
      ...
      if ((key === 'limit' || key === 'pageSize' || key === 'per_page' || key === 'count') &&
      ...
      if ((key === 'page' || key === 'offset' || key === 'skip') &&
          parseInt(value) < 0) {
...
async function checkBodyDedup(req, ip) {
  ...
  if (count === null) count = memoryIncr(_bodyHashStore, key, CONFIG.BODY_DEDUP_WINDOW_S * 1000);

  if (count > CONFIG.BODY_DEDUP_MAX) {
    await blockIP(ip, CONFIG.BODY_DEDUP_BLOCK_S, 'ATTACK_BODY_DEDUP',
...
async function checkMissingSecFetchOnMutation(req, ip) {
  ...
  if (!hasSecFetch) {
    ...
    if (velocity > 5) {
      await blockIP(ip, CONFIG.L10_BLOCK_S, 'ATTACK_NO_SEC_FETCH',
...
const advancedAttackLimiter = async (req, res, next) => {
  try {
    ...
    if (checkL12_Honeypot(req)) {
      await blockIP(ip, CONFIG.L12_BLOCK_S, 'ATTACK_L12_HONEYPOT',
        `Honeypot path: ${path}`);
    ...
    if (checkL4_BotUA(req)) {
      await blockIP(ip, CONFIG.L4_BLOCK_S, 'ATTACK_L4_BOT_UA',
    ...
    if (checkL9_HeaderSpoof(req)) {
      await blockIP(ip, CONFIG.L9_BLOCK_S, 'ATTACK_L9_HEADER_SPOOF',
    ...
    if (queryAbuse) {
      await blockIP(ip, 3600, 'ATTACK_QUERY_ABUSE',
        `Query param abuse: ${queryAbuse}`);
    ...
    if (checkL7_SlowLoris(ip)) {
      await blockIP(ip, CONFIG.L7_BLOCK_S, 'ATTACK_L7_SLOW_LORIS',
...
function getAttackStats() {
  return {
    ...
    activeConnections: Array.from(_connCountStore.entries()).reduce((sum, [, v]) => sum + v.count, 0),
    concurrentRequests: Array.from(_concurrentStore.entries()).reduce((sum, [, v]) => sum + v, 0),
