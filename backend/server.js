...
const { taskQueue, getUserActiveJobs, incrementUserActiveJobs } = require('./services/taskQueue');
...
const { computeCombinedContentHash, createBundle, buildBundleS3Key } = require('./services/multiFileService');
...
const { advancedAttackLimiter, notFoundAttackTracker, recordAuthFailure } = require('./middleware/advancedRateLimiter');
...
const {
  PasswordValidator,
  AccountLockout
} = require('./security/passwordValidator');
const { initializeRedis, getRedisClient, disconnectRedis } = require('./redisClient');

const { connectDatabase, setupEventHandlers, getStatus, disconnect } = require('./config/database');
...
app.use((req, res, next) => {
  ...
  res.setTimeout(timeout, () => {
    if (!res.headersSent) {
      res.status(408).json({ success: false, error: { code: 'REQUEST_TIMEOUT', message: 'Request timed out. Please try again.' } });
    }
  });
  next();
});
...
const BANNED_BODY_FIELDS = ['role', 'isAdmin', 'isSuperAdmin', 'subscriptionTier',
  'subscriptionStatus', 'tokenVersion', 'emailVerified', 'isActive', '__proto__',
...
const refreshLimiter = createEndpointLimiter(10, 60 * 60 * 1000, 'Too many token refresh attempts');
...
// ===== CORS =====
const ALLOWED_ORIGINS = (process.env.CORS_ORIGINS || "http://localhost:5173,http://localhost:5174,https://vayrex.netlify.app").split(',').map(s => s.trim());
...
app.use(cors({
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'X-CSRF-Token', 'X-Requested-With', 'Bypass-Tunnel-Reminder', 'ngrok-skip-browser-warning'],
  ...
  for (const [key, value] of Object.entries(req.query)) {
    ...
    if (['limit', 'pagesize', 'per_page', 'count'].includes(key.toLowerCase())) {
      ...
      if (!isNaN(num) && num > QUERY_MAX_LIMIT) {
        ...
        Logger.warn('Excessive limit capped', { ip: req.ip, path: req.path, original: value, capped: QUERY_MAX_LIMIT });
      }
      if (!isNaN(num) && num < 1) {
...
app.get('/api/ai/job-status/:jobId', authenticateToken, async (req, res) => {
  try {
    ...
    if (!job) {
      ...
      const completedQuestionsCount = await Question.countDocuments({ batchId: `job-${jobId}`, userId: req.user.id });
      if (completedQuestionsCount > 0) {
        const firstQuestion = await Question.findOne({ batchId: `job-${jobId}`, userId: req.user.id }).select('topic').lean();
        return res.json({
          success: true,
          jobId,
          status: 'completed',
...
function _summaryJobCreate(userId) {
  ...
  _summaryJobStore.set(jobId, {
    userId, status: 'processing', title: null, chapters: [], client: null, error: null
  });
...
function _summaryJobEmit(jobId, event) {
  ...
  if (event.type === 'title')    { job.title = event.title; }
  else if (event.type === 'chapter') { job.chapters.push(event.chapter); }
  else if (event.type === 'complete'){ job.status = 'completed'; }
  else if (event.type === 'error')   { job.status = 'failed'; job.error = event.message; }
...
app.get('/api/ai/job-status/stream/:jobId', authenticateToken, async (req, res) => {
  ...
  if (currentSseCount >= MAX_SSE_PER_USER) {
    return res.status(429).json({
      ...
      error: {
        ...
        message: 'Too many active status streams. Close existing connections before opening new ones.',
  ...
  const sendStatus = async () => {
    ...
    try {
      ...
      if (String(job.data.userId) !== String(req.user.id)) {
        res.write(JSON.stringify({ success: false, error: { code: 'FORBIDDEN', message: 'Forbidden' } }) + '\n');
        res.end();
        closed = true;
        return;
      }
      ...
      res.write(JSON.stringify({ success: false, error: { code: 'SERVER_ERROR', message: 'Internal server error' } }) + '\n');
      res.end();
      closed = true;
    }
  };
...
const adminRateLimiter = createEndpointLimiter(20, 60 * 1000, 'Too many admin requests. Please slow down');
...
app.use((err, req, res, next) => {
  ...
  if (err.code === 'EBADCSRFTOKEN') {
    Logger.error('CSRF token validation failed - DETAILED DEBUG', {
      ...
      tokenValue: req.headers['x-csrf-token'] ? req.headers['x-csrf-token'].substring(0, 10) + '...' : 'NONE',
...
try {
  ...
  } else if (fs.existsSync('./serviceAccountKey.json') && process.env.NODE_ENV !== 'production') {
        ...
        try {
          ...
          if (pptxErr.message.includes('OLD_PPT_FORMAT')) {
            return res.status(400).json({
              ...
              error: {
                code: 'OLD_PPT_FORMAT',
                message: 'Old .ppt files are not supported. Please convert to .pptx format (Open in PowerPoint → Save As → .pptx) or use PDF/Word format instead.',
                timestamp: new Date().toISOString()
              }
            });
          }
...
app.delete('/api/uploads/id/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res.status(400).json({
        success: false,
        error: { code: 'INVALID_ID', message: 'Invalid upload ID', timestamp: new Date().toISOString() }
      });
    }
    ...
    if (!upload) {
      return res.status(404).json({
        success: false,
        error: { code: 'UPLOAD_NOT_FOUND', message: 'Upload not found', timestamp: new Date().toISOString() }
      });
    }

    // ── S3 cleanup ──────────────────────────────────────────────────
    const s3Keys = [upload.s3FileKey, upload.s3BundleKey, upload.s3BackupKey].filter(Boolean);
    await Promise.allSettled(s3Keys.map(key =>
      s3Client.send(new DeleteObjectCommand({ Bucket: S3_BUCKET, Key: key }))
        .then(() => Logger.info('S3 object deleted', { key }))
        .catch(err => Logger.warn('S3 delete failed (non-critical)', { key, error: err.message }))
    ));
    ...
    return res.status(500).json({
      success: false,
      error: { code: 'DELETE_ERROR', message: 'Failed to delete upload', timestamp: new Date().toISOString() }
    });
  }
});
...
app.delete("/api/uploads/:topic/:filename", authenticateToken, async (req, res) => {
  try {
    ...
    if (!topic || !filename) {
      return res.status(400).json({ success: false, error: { code: 'MISSING_PARAMS', message: 'Topic and filename are required' } });
    }

    if (typeof topic !== 'string' || typeof filename !== 'string') {
      return res.status(400).json({ success: false, error: { code: 'INVALID_PARAMS', message: 'Parameters must be strings' } });
    }
    ...
    Logger.info('Single file deleted', { userId, topic: sanitizedTopic, filename: sanitizedFilename });
    ...
    res.json({
      ...
      data: {
        ...
        s3Warnings: s3Errors.length > 0 ? `${s3Errors.length} S3 file(s) could not be deleted` : null
      }
    });
  } catch (error) {
    Logger.error('Delete single file error', { error: error.message, userId: req.user?.id });
    res.status(500).json({ success: false, error: { code: 'DELETE_FILE_ERROR', message: 'Failed to delete file' } });
  }
});
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        Logger.error('Bulk upload file error', { fileName: file.name, error: fileErr.message });
        job.files[i].status = 'failed';
    ...
    job.status = job.files.every(f => f.status === 'completed') ? 'completed'
      : job.files.every(f => f.status === 'failed') ? 'failed' : 'partial';
    await job.save();
    ...
    Logger.info('Bulk upload completed', { userId, topic: sanitizedTopic, files: files.length, questions: totalQuestions });
    ...
    res.status(500).json({ success: false, error: { code: 'BULK_UPLOAD_ERROR', message: 'Failed to process bulk upload' } });
  }
});
...
app.get('/api/upload/bulk/:jobId', authenticateToken, async (req, res) => {
  try {
    const job = await BulkUploadJob.findOne({ _id: req.params.jobId, userId: req.user.id }).lean();
    if (!job) {
      return res.status(404).json({ success: false, error: { code: 'JOB_NOT_FOUND', message: 'Upload job not found' } });
    }
    res.json({ success: true, data: job });
  } catch (err) {
    res.status(500).json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to fetch job status' } });
  }
});
...
async function extractTextFromUpload(tempFilePath, originalName) {
  ...
  try { return fs.readFileSync(tempFilePath, 'utf8'); } catch (_) { return ''; }
}
...
app.post('/api/export/pdf',
  ...
  async (req, res) => {
    try {
      ...
      if (!user.limits.pdfExport) {
        return res.status(403).json({
          ...
          error: {
            ...
            message: 'PDF export is available on Pro plan and above. Please upgrade your subscription.',
            requiredTier: 'pro'
          }
        });
      }

      const { topic, difficulty, includeAnswers = true, format = 'questions', fontSize = 11, fontFamily = 'Helvetica' } = req.body;
      ...
      const safeTopicName = topic.trim().replace(/[^a-zA-Z0-9-_ ]/g, '').substring(0, 50);
      ...
      res.status(500).json({ success: false, error: { code: 'PDF_EXPORT_ERROR', message: 'Failed to generate PDF' } });
    }
  });
    ...
    try {
      ...
      if (!user.limits.noteSummary) {
        return res.status(403).json({
          ...
          error: {
            code: 'TIER_RESTRICTED',
            message: 'Note summarization is available on Pro plan and above. Please upgrade your subscription.',
            requiredTier: 'pro'
          }
        });
      }
      ...
      if (req.files && req.files.file) {
        ...
        return res.status(400).json({ success: false, error: { code: 'NO_CONTENT', message: 'Provide a file or text to summarize' } });
      }

      if (!text || text.trim().length < 50) {
        return res.status(400).json({ success: false, error: { code: 'INSUFFICIENT_TEXT', message: 'Text is too short to summarize (minimum 50 characters)' } });
      }
      ...
      Logger.info('Note summarized', { userId, originalLength: text.length, summaryLength: summary.text.length });
    } catch (err) {
      Logger.error('Note summarization error', { error: err.message });
      res.status(500).json({ success: false, error: { code: 'SUMMARIZE_ERROR', message: 'Failed to summarize notes' } });
    }
  });
    ...
    try {
      ...
      if (!user.limits?.noteSummary) {
        return res.status(403).json({
          ...
          error: {
            code: 'TIER_RESTRICTED',
            message: 'AI note summary is available on Pro plan and above. Please upgrade your subscription.',
            requiredTier: 'pro'
          }
        });
      }
...
async function _uploadSummaryFileToS3(userId, fileBuffer, fileName) {
  ...
  await s3Client.send(new PutObjectCommand({
    Bucket: S3_BUCKET, Key, Body: fileBuffer,
...
app.post('/api/ai/summarize-start',
  authenticateToken,
  checkTokenLimit(1500),
  trackTokenUsage,
  async (req, res) => {
    try {
      ...
      if (!user.limits?.noteSummary) {
        return res.status(403).json({
          success: false,
          error: { code: 'TIER_RESTRICTED', message: 'AI note summary is available on Pro plan and above.', requiredTier: 'pro' }
        });
      }
      ...
      if (!hasFiles && !hasText) {
        return res.status(400).json({
          success: false,
          error: { code: 'NO_CONTENT', message: 'Provide a file or at least 50 characters of text.' }
        });
      }
      ...
      if (uploadedFiles.length > 1) {
        ...
        if (uploadedFiles.length > ABSOLUTE_MAX_FILES) {
          return res.status(413).json({
            success: false,
            error: { code: 'FILE_COUNT_EXCEEDED', message: `Maximum ${ABSOLUTE_MAX_FILES} files per upload. You sent ${uploadedFiles.length}.` }
          });
        }
        ...
        await new Promise((resolve, reject) => {
          validateMultiFileUpload(req, { status: (code) => ({ json: (body) => reject(Object.assign(new Error(body?.error?.message || 'File validation failed'), { statusCode: code, responseBody: body })) }) }, resolve);
        });
      }
      ...
      if (hasFiles) {
        if (uploadedFiles.length === 1) {
          ...
          const perFileHashes = uploadedFiles.map(f => crypto.createHash('sha256').update(f.data).digest('hex'));
          perFileHashes.sort();
      ...
      if (existingSession && existingSession.status === 'complete') {
        Logger.info('summarize-start: dedup hit, returning existing session', {
          sessionId: existingSession._id, userId: req.user.id
        });
        return res.json({
          success: true,
          deduplicated: true,
      ...
      try {
        ...
        Logger.warn('summarize-start: S3 upload failed, continuing without S3', { error: s3Err.message });
      }
      ...
      Logger.info('summarize-start job created', { jobId, sessionId, userId: req.user.id, hasFiles, fileCount });
      ...
      const filesToProcess = hasFiles ? uploadedFiles.map(f => ({ data: Buffer.from(f.data), name: f.name })) : [];
      ...
      setImmediate(async () => {
        try {
          ...
          const streamArgs = {
            ...
            fileName: hasFiles ? (filesToProcess.length === 1 ? filesToProcess[0].name : filesToProcess.map(f => f.name).join(' + ')) : 'study-notes',
            onTitle: async (title) => {
              _summaryJobEmit(jobId, { type: 'title', title });
              try { await SummarySession.updateOne({ _id: sessionId }, { title }); } catch {}
            },
            onChapter: async (chapter) => {
              _summaryJobEmit(jobId, { type: 'chapter', chapter });
              try {
                await SummarySession.updateOne({ _id: sessionId }, { $push: { chapters: chapter } });
              } catch {}
            },
            onComplete: async (totalChapters) => {
              _summaryJobEmit(jobId, { type: 'complete', totalChapters });
              try {
                await SummarySession.updateOne({ _id: sessionId }, {
                  status: 'complete', totalExpectedChunks: totalChapters
                });
              } catch {}
            },
          ...
          _summaryJobEmit(jobId, { type: 'error', message: bgErr.message || 'Processing failed' });
          try {
            await SummarySession.updateOne({ _id: sessionId }, { status: 'failed' });
          } catch {}
        }
...
app.get('/api/ai/summarize-stream/:jobId',
  authenticateToken,
  async (req, res) => {
    const jobId = decodeURIComponent(req.params.jobId || '');
    if (!jobId) {
    ...
    if (job.status === 'failed') {
      res.write(JSON.stringify({ type: 'error', message: job.error || 'Processing failed' }) + '\n');
      res.end();
      return;
    }
...
app.get('/api/ai/summary-sessions/:id', authenticateToken, async (req, res) => {
  try {
    const session = await SummarySession.findOne({ _id: req.params.id, userId: req.user.id }).lean();
...
app.put('/api/ai/summary-sessions/:id/chat', authenticateToken, async (req, res) => {
  try {
    ...
    if (!Array.isArray(messages) || messages.length === 0) {
      return res.status(400).json({ success: false, error: { code: 'INVALID', message: 'messages array required' } });
    }
    // Sanitise and push
...
app.put('/api/ai/summary-sessions/:id/annotations', authenticateToken, async (req, res) => {
  try {
    ...
    if (highlights !== undefined && typeof highlights === 'object') update.highlights = highlights;
    if (userNotes !== undefined && typeof userNotes === 'object') update.userNotes = userNotes;
    if (Object.keys(update).length === 0) {
      return res.status(400).json({ success: false, error: { code: 'INVALID', message: 'highlights or userNotes object required' } });
    }
    await SummarySession.updateOne({ _id: req.params.id, userId: req.user.id }, { $set: update });
    res.json({ success: true });
  } catch (err) {
...
app.delete('/api/ai/summary-sessions/:id', authenticateToken, async (req, res) => {
  try {
    const session = await SummarySession.findOneAndDelete({ _id: req.params.id, userId: req.user.id });
    if (!session) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Session not found' } });
    // Clean up S3 file (fire-and-forget)
    if (session.s3Key) {
      s3Client.send(new DeleteObjectCommand({ Bucket: S3_BUCKET, Key: session.s3Key })).catch(() => {});
    }
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/ai/course-outline/parse',
  authenticateToken,
  ...
  async (req, res) => {
    try {
      ...
      if (!user.limits?.noteSummary) {
        return res.status(403).json({
          success: false,
          error: { code: 'TIER_RESTRICTED', message: 'Course outline notes are available on Pro plan and above.', requiredTier: 'pro' }
        });
      }
      ...
      const extractTextFromFile = async (file) => {
        ...
        try {
          if (['.jpg', '.jpeg', '.png', '.webp'].includes(ext)) {
            ...
            const ocrResult = await imageOcrService.ocrImage(file.data, file.name, { document: true });
          ...
          Logger.warn('course-outline/parse: file extraction error', {
            fileName: file.name, ext, error: parseErr.message
          });
      ...
      if (fileList.length > 0) {
        ...
        if (textParts.length === 0 && unsupported.length > 0) {
          return res.status(400).json({
            success: false,
            error: { code: 'UNSUPPORTED_FILE', message: 'Supported formats: PDF, DOCX, TXT, JPG, PNG, WEBP' }
          });
        }
        ...
        return res.status(400).json({
          success: false,
          error: { code: 'NO_CONTENT', message: 'Provide a file or at least 30 characters of text.' }
        });
      }
      ...
      if (rawText.trim().length < 30) {
        return res.status(400).json({
          success: false,
          error: { code: 'INSUFFICIENT_CONTENT', message: 'Could not extract enough text. Ensure the content is readable.' }
        });
      }
      ...
      // Check for dedup
      const existingSession = await SummarySession.findByFileHash(req.user.id, analysis.contentHash);
      const deduplicated = existingSession && existingSession.status === 'complete' && existingSession.sessionType === 'course_outline';
      ...
      res.status(500).json({ success: false, error: { code: 'SERVER_ERROR', message: 'An internal error occurred. Please try again.' } });
    }
  }
);

// ── POST /api/ai/course-outline/generate — Start outline generation ──
app.post('/api/ai/course-outline/generate',
  authenticateToken,
  ...
  async (req, res) => {
    try {
      ...
      for (let ci = 0; ci < chapters.length; ci++) {
        ...
        if (!ch.chapterTitle) {
          Logger.warn('course-outline/generate: chapter missing chapterTitle, assigning default', {
            chapterIndex: ci, keys: Object.keys(ch), weekNumber: ch.weekNumber
          });
        ...
        if (ch.subTopics.length > 25) {
          return res.status(400).json({
            success: false,
            error: { code: 'INVALID', message: `Chapter "${ch.chapterTitle.substring(0, 50)}" has too many sub-topics (max 25).` }
          });
        }
      ...
      if (totalSubTopics > 300) {
        return res.status(400).json({
          success: false,
          error: { code: 'INVALID', message: 'Total sub-topics across all chapters cannot exceed 300.' }
        });
      }

      // Sanitize inputs
      const safeCourseName = (courseName || 'Course Notes').substring(0, 200).trim();
      const safeDepthTier = ['full', 'standard', 'condensed'].includes(depthTier) ? depthTier : 'standard';
      const safeHash = contentHash || crypto.createHash('sha256').update(JSON.stringify(chapters)).digest('hex');
      ...
      Logger.info('course-outline/generate job created', {
        jobId, sessionId, userId: req.user.id,
        chapters: chapters.length, depthTier: safeDepthTier
      });
      ...
      setImmediate(async () => {
        try {
          ...
          await outlineGenerationService.generateOutlineContent({
            ...
            onProgress: (event) => {
              ...
              if (event.type === 'chapter_overview') {
                ...
                SummarySession.findById(sessionId).lean().then(sess => {
          ...
          Logger.error('course-outline/generate background error', { jobId, error: bgErr.message });
          _summaryJobEmit(jobId, { type: 'error', message: 'Generation encountered an error. Please try again.' });
          try {
            await SummarySession.updateOne({ _id: sessionId }, { status: 'failed' });
          } catch {}
        }
...
app.post('/api/ai/course-outline/:sessionId/export',
  authenticateToken,
  async (req, res) => {
    try {
      if (!mongoose.Types.ObjectId.isValid(req.params.sessionId)) {
        return res.status(400).json({ success: false, error: { code: 'INVALID_ID', message: 'Invalid session ID format.' } });
      }
      ...
      if (session.status === 'streaming') {
        return res.status(400).json({
          success: false,
          error: { code: 'STILL_GENERATING', message: 'Notes are still being generated. Please wait until generation completes.' }
        });
      }
...
app.post('/api/ai/summary/:sessionId/export',
  authenticateToken,
  async (req, res) => {
    try {
      if (!mongoose.Types.ObjectId.isValid(req.params.sessionId)) {
      ...
      if (session.status === 'streaming') {
        return res.status(400).json({
          success: false,
          error: { code: 'STILL_GENERATING', message: 'Summary is still being generated. Please wait until generation completes.' }
        });
      }
      ...
      if (session.s3Key && (isPptx || isPdf || isDocx || isImage)) {
        try {
          ...
          if (isPptx) {
            ...
            const slideImages = (pptxResult.slides || []).flatMap(
              s => (s.images || []).map(img => ({
                ...img, slideNumber: s.slideNumber,
                position: s.slideNumber,
                totalPositions: (pptxResult.slides || []).length,
                sourceFormat: 'pptx'
              }))
            );
            const allImages = (pptxResult.images || []).filter(img => img.buffer).map(img => ({
              ...img, sourceFormat: 'pptx'
            }));
          ...
          Logger.warn('summary export: failed to extract source images, continuing without', { error: imgErr.message });
        }
      }
...
app.post('/api/ai/course-outline/:sessionId/retry',
  authenticateToken,
  checkTokenLimit(1500),
  trackTokenUsage,
  async (req, res) => {
    try {
      ...
      if (session.status === 'streaming') {
        return res.status(400).json({
          success: false,
          error: { code: 'STILL_GENERATING', message: 'Generation is still in progress. Please wait.' }
        });
      }
      ...
      Logger.info('course-outline/retry started', {
        jobId, sessionId: req.params.sessionId, userId: req.user.id
      });
      ...
      setImmediate(async () => {
        try {
          ...
          Logger.error('course-outline/retry background error', { jobId, error: bgErr.message });
          _summaryJobEmit(jobId, { type: 'error', message: 'Retry encountered an error. Please try again.' });
          try {
            await SummarySession.updateOne({ _id: req.params.sessionId }, { status: 'partial' });
          } catch {}
        }
      });
    } catch (err) {
...
app.post('/api/ai/summary-sessions/:id/generate-quiz',
  authenticateToken,
  ...
  async (req, res) => {
    try {
      ...
      if (!session) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Session not found' } });
      if (!session.s3Key) {
        return res.status(400).json({ success: false, error: { code: 'NO_FILE', message: 'Original source file not available for quiz generation.' } });
      }
      ...
      try {
        ...
        Logger.error('generate-quiz-from-summary: S3 download failed', { error: s3Err.message });
        return res.status(500).json({ success: false, error: { code: 'S3_ERROR', message: 'Could not retrieve original file.' } });
      }
      ...
      if (ext === '.pdf')  { extractedContent = await require('./parsers').parsePdf(fileBuffer); }
      else if (ext === '.docx') { extractedContent = await require('./parsers').parseDocx(fileBuffer); }
      else if (ext === '.pptx' || ext === '.ppt') { extractedContent = await batchProcessingService.extractFullContent(fileBuffer, ext); }
      else { extractedContent = fileBuffer.toString('utf8'); }

      if (!extractedContent || extractedContent.trim().length < 50) {
        return res.status(400).json({ success: false, error: { code: 'INSUFFICIENT_CONTENT', message: 'Not enough content in the source file to generate questions.' } });
      }
      ...
      if (!questions || questions.length === 0) {
        return res.status(400).json({ success: false, error: { code: 'NO_QUESTIONS', message: 'Could not generate questions from this content.' } });
      }

      // Save questions to DB
      const savedQuestions = await Question.insertMany(
        questions.map(q => ({ ...q, userId: req.user.id, topic: topic.toLowerCase(), sourceFile: session.sourceFileName }))
      );
...
app.post('/api/ai/summary-sessions/:id/quick-check',
  authenticateToken,
  checkTokenLimit(500),
  trackTokenUsage,
  async (req, res) => {
    try {
      ...
      if (!session.chapters || session.chapters.length === 0) {
        return res.status(400).json({ success: false, error: { code: 'NO_CONTENT', message: 'No chapters available for comprehension check.' } });
      }

      // Build a text excerpt from all chapters
      const chapterText = session.chapters.map(ch => {
        const teaching = (ch.coreTeaching || []).map(s => `${s.sectionTitle}: ${s.content}`).join('\n');
...
app.get('/api/ai/summary-sessions/:id/threads', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ success: true, threads, activeChatThreadId: session.activeChatThreadId || null });
  } catch (err) {
...
app.post('/api/ai/summary-sessions/:id/threads', authenticateToken, async (req, res) => {
  try {
    ...
    const threadCount = await ChatThread.countDocuments({ sessionId: req.params.id, userId: req.user.id });
    if (threadCount >= 20) {
      return res.status(400).json({ success: false, error: { code: 'THREAD_LIMIT', message: 'You\'ve reached the 20-chat limit for this session. Delete an old chat to create a new one.' } });
    }
...
app.get('/api/ai/summary-sessions/:id/threads/:tid', authenticateToken, async (req, res) => {
  try {
    const thread = await ChatThread.findOne({ _id: req.params.tid, sessionId: req.params.id, userId: req.user.id }).lean();
    if (!thread) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Thread not found' } });
...
app.put('/api/ai/summary-sessions/:id/threads/:tid/messages', authenticateToken, async (req, res) => {
  try {
    ...
    const thread = await ChatThread.findOne({ _id: req.params.tid, sessionId: req.params.id, userId: req.user.id });
    if (!thread) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Thread not found' } });

    // Auto-title from first user message — use AI to generate a short memorable title
    let titleUpdate = {};
    if (thread.title === 'New Chat') {
      const firstUser = toAdd.find(m => m.role === 'user') ||
                        thread.messages.find(m => m.role === 'user');
      if (firstUser) {
        // Fire AI title generation in the background so it doesn't block the response
        aiService.generateShortTitle(firstUser.content, firstUser.content.replace(/\s+/g, ' ').trim().slice(0, 45))
          .then(aiTitle => {
            ChatThread.updateOne({ _id: req.params.tid }, { $set: { title: aiTitle } }).catch(() => {});
          })
          .catch(() => {});
    ...
    const trimmed = combined.length > thread.maxMessages
...
app.patch('/api/ai/summary-sessions/:id/threads/:tid', authenticateToken, async (req, res) => {
  try {
    ...
    if (!title || typeof title !== 'string') {
      return res.status(400).json({ success: false, error: { code: 'INVALID', message: 'title required' } });
    }
    const result = await ChatThread.updateOne(
      { _id: req.params.tid, sessionId: req.params.id, userId: req.user.id },
      { title: title.trim().slice(0, 100) }
    );
    if (!result.matchedCount) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Thread not found' } });
    res.json({ success: true });
  } catch (err) {
...
app.delete('/api/ai/summary-sessions/:id/threads/:tid', authenticateToken, async (req, res) => {
  try {
    const result = await ChatThread.deleteOne({ _id: req.params.tid, sessionId: req.params.id, userId: req.user.id });
    if (!result.deletedCount) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Thread not found' } });
...
app.patch('/api/ai/summary-sessions/:id/active-thread', authenticateToken, async (req, res) => {
  try {
...
// ===== MAIN ENDPOINT =====
app.post("/api/ai/parse-questions",
  authenticateToken,
  checkUploadLimit,
  checkFileSize,
  checkStorageLimit,
    ...
    try {
      ...
      const cleanedQuestions = rawQuestions
        .map((q, index) => ({
          ...
          explanation: (q.explanation || '').trim(),
          difficulty: q.difficulty || 'Medium',
          questionType: q.questionType || (q.options?.length > 0 ? 'multiple-choice' : 'essay'),
          userId,
          topic,
          sourceFile: fileName
        }))
...
app.post("/api/ai/grade-answer",
  ...
  async (req, res) => {
    try {
      ...
      const grade = await aiService.gradeAnswer(questionText, userAnswer, correctAnswer || null);
...
app.post('/api/ai/chat',
  ...
  async (req, res) => {
    try {
      const { messages, context = 'academic', studyData = null, model: clientModel = null } = req.body;
      ...
      const result = await aiService.chat(messages, context, clientModel || req.user?.limits?.aiModel || null);
...
const conditionalFileValidation = (req, res, next) => {
  const fileCount = req.files ? (Array.isArray(req.files.file) ? req.files.file.length : (req.files.file ? 1 : 0)) : 0;
  ...
  if (fileCount > ABSOLUTE_MAX_FILES) {
    Logger.warn('File upload rejected - absolute file count cap exceeded', { fileCount, max: ABSOLUTE_MAX_FILES });
    return res.status(413).json({
      success: false,
      error: {
        code: 'FILE_COUNT_EXCEEDED',
...
app.post("/api/ai/generate-from-notes",
  authenticateToken,
  checkUploadLimit,
  checkFileSize,
  checkStorageLimit,
  ...
  async (req, res) => {
    try {
      ...
      const enforceQueueLimits = async () => {
        ...
        const globalActive = (globalCounts.active || 0) + (globalCounts.waiting || 0) + (globalCounts.delayed || 0);
        if (globalActive >= maxGlobalJobs) {
          ...
          return {
            ...
            error: {
              ...
              message: 'The system is experiencing unusually high demand. Please try again shortly.',
      ...
      if (req.validatedFiles && req.validatedFiles.length > 1) {
        ...
        try {
          ...
          s3Client.send(bundleUploadCmd)
            ...
            .catch(async (err) => {
              Logger.error('[S3 FAIL] Multi-file ZIP bundle upload failed', {
                error: err.message, s3Key: bundleS3Key, userId: req.user.id
              });
              try {
                const _redis = getRedisClient();
          ...
          const job = await taskQueue.add(`quiz-${userId}-${Date.now()}`, {
            userId: req.user.id,
            topic,
            difficulty,
            questionsCount,
          ...
          try {
            ...
            Logger.error('Failed to increment active jobs after multi-file enqueue', { error: incrErr.message });
          ...
          Logger.error('Error enqueuing multi-file quiz job:', { error: err.message, stack: err.stack });
          return res.status(500).json({
            success: false,
            error: {
              code: 'QUEUE_ERROR',
      ...
      if (req.validatedFile || (req.validatedFiles && req.validatedFiles.length === 1)) {
        ...
        try {
          ...
          const s3FileKey = await uploadToS3(fileBuffer, fileName, file.mimetype, req.user.id).then(res => res.Key);
          Logger.info(`[S3 SUCCESS] File successfully uploaded to S3`, { s3FileKey, userId: req.user.id });
          ...
          try {
            ...
            Logger.error('Failed to increment active jobs after enqueue, removing job', { error: incrErr.message });
            await job.remove().catch(() => { });
            throw incrErr;
          }
      ...
      Logger.error('Fatal error in quiz generation route:', { error: err.message, stack: err.stack });
      if (!res.headersSent) {
        return res.status(500).json({
          success: false,
          error: {
...
app.get("/api/user/quiz", authenticateToken, async (req, res) => {
  try {
    ...
    const projectStage = mode === "Practice"
      ? {} // Include all fields
      : { $project: { correctAnswer: 0, explanation: 0 } }; // Only exclude - MongoDB preserves _id automatically
  ...
  try {
    ...
    const graded = validAnswers.map(a => {
      ...
      if (qType === 'fill-in-blank') {
        ...
        isCorrect = a.selectedIndex !== null &&
      ...
      return {
        ...
        correctText: qType === 'fill-in-blank' ? (q.blankAnswer || null)
                   : qType === 'theory'        ? (q.modelAnswer  || null)
                   : null,
        isCorrect,
      };
    });
    ...
    User.findByIdAndUpdate(userId, {
      ...
      Logger.error('Failed to increment quizzesTaken', { userId: userId.toString(), error: err.message });
    });
...
app.get("/api/results", authenticateToken, async (req, res) => {
  try {
    ...
    return res.json(ResponseFormatter.successPaginated(results, {
  ...
  try {
    ...
    const enrichedAnswers = result.answers.map((answer, index) => {
      ...
      return {
        ...
        userSelectedIndex: answer.selectedIndex !== null && answer.selectedIndex !== undefined
    ...
    const topicBreakdown = Object.keys(topicStats).map(topic => ({
      ...
      percentageInTopic: Math.round((topicStats[topic].correct / topicStats[topic].total) * 100)
    }));
  ...
  try {
    ...
    try {
      ...
      if (cfgLimits) {
        ...
        ['filesPerUpload', 'maxFileSizeMB', 'questionsPerUpload',
         'uploadsPerMonth', 'uploadsPerDay', 'tokensPerMonth',
         'tokensPerRequest', 'maxStorageMB'].forEach(f => {
          if (cfgLimits[f] !== undefined) verifyLiveLimits[f] = cfgLimits[f];
        });
      }
    } catch (_limitsErr) { /* non-fatal — fall back to stored limits */ }
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          ticketInfo: 'Please save this ticket ID for your records. You can use it to track your inquiry.'
        },
        error: null,
        timestamp: new Date().toISOString()
      });
  ...
  try {
    ...
    try {
      ...
      if (tierLimitsFromConfig) {
        ...
        ['filesPerUpload', 'maxFileSizeMB', 'questionsPerUpload',
         'uploadsPerMonth', 'uploadsPerDay', 'tokensPerMonth',
         'tokensPerRequest', 'maxStorageMB'].forEach(override);
      }
    } catch (limitsErr) {
...
app.post('/api/user/upgrade', authenticateToken, async (req, res) => {
  try {
    ...
    if (tierOrder[tier] <= tierOrder[user.subscriptionTier]) {
      return res.status(400).json({
        success: false,
        error: { code: 'INVALID_UPGRADE', message: 'Cannot upgrade to same or lower tier. Use downgrade endpoint instead.' }
      });
    }
    ...
    const priceUSDCents = billingCycle === 'yearly' ? tierConfig.yearlyUSD : tierConfig.monthlyUSD;
    ...
    const initTransaction = () => new Promise((resolve, reject) => {
      const options = {
    ...
    Logger.info('Payment initialized', { userId: user._id, tier, amountKobo: amountInKobo, priceUSDCents, ngnRate, reference: paystackResponse.data.reference });
  ...
  try {
    ...
    switch (event.event) {
      case 'charge.success': {
        ...
        Logger.info('Subscription activated via webhook', { userId, tier, expiry, reference });
        break;
      }
      ...
      case 'charge.failed': {
        ...
        if (user) {
          user.subscriptionStatus = event.event === 'charge.failed' ? 'past_due' : 'cancelled';
          await user.save();
...
app.get('/api/paystack/verify/:reference', authenticateToken, async (req, res) => {
  try {
    ...
    if (!paystackSecretKey) {
      return res.status(503).json({ success: false, error: { code: 'PAYMENT_UNAVAILABLE', message: 'Payment not configured' } });
    }
    ...
    const ownershipByUid = paymentOwnerUserId && paymentOwnerUserId.toString() === req.user.id.toString();
    const ownershipByEmail = customerEmail && authenticatedUserEmail &&
    ...
    res.status(500).json({ success: false, error: { code: 'VERIFY_ERROR', message: 'Failed to verify payment' } });
  }
});
...
app.post('/api/user/cancel-subscription', authenticateToken, async (req, res) => {
  try {
    ...
    if (user.subscriptionTier === 'free') {
      return res.status(400).json({ success: false, error: { code: 'ALREADY_FREE', message: 'You are already on the free plan' } });
    }
    ...
    if (user.paystackSubscriptionCode && process.env.PAYSTACK_SECRET_KEY) {
      try {
        const https = require('https');
        const disableSubscription = () => new Promise((resolve, reject) => {
        ...
        Logger.info('Paystack subscription disabled', { userId: user._id, code: user.paystackSubscriptionCode });
      } catch (paystackErr) {
    ...
    res.status(500).json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to cancel subscription' } });
  }
});
